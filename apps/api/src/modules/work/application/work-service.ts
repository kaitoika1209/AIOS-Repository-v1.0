...
export class WorkService {
  ...
  private execute(context:CommandContext,type:string,permission:string,workId:string,operation:()=>Promise<unknown>):Promise<unknown>{
    return this.dependencies.transactions.transaction(async()=>{this.requireScope(context.principal,context.organizationId);const prior=await this.dependencies.commands.result(context.commandId);if(prior!==undefined)return prior;await this.dependencies.audit.record({commandId:context.commandId,organizationId:context.organizationId,workId,membershipId:context.principal.membershipId,permission,outcome:'Allow',reasonCode:'AUTHORIZED'});const result=await operation();await this.dependencies.commands.record(context.commandId,context.organizationId,type,result);return result;});
  }
  private requireScope(principal:HumanMemberPrincipal,organizationId:string):void {if(principal.organizationId!==organizationId)throw new WorkPermissionDenied();}
