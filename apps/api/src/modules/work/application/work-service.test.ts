...
function harness(state:WorkState|undefined=started){
  ...
  let event=0; const service=new WorkService({transactions:{transaction:(operation)=>operation()},repository,commands,events,audit,clock:{now:()=>new Date('2026-07-31T00:00:00Z')},ids:{eventId:()=>`event-${++event}`}});
  return {service,save,appended,state:()=>saved};
}

describe('WorkService',()=>{
  it('atomically completes Work and appends one WorkCompleted event',async()=>{const h=harness();await h.service.complete({commandId:'cmd-1',organizationId:'org-a',principal},{workId:'work-a',expectedVersion:2,completionSummary:'Done'});expect(h.state()?.status).toBe('Completed');expect(h.appended).toHaveLength(1);expect(h.appended[0]).toMatchObject({eventType:'WorkCompleted',aggregateVersion:3});});
