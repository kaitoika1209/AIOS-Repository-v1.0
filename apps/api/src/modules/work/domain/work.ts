...
export class Work {
  ...
  complete(actor:WorkActor,summary:string|undefined,now:Date):void { this.requireStatus('InProgress'); this.bump({...this.current,status:'Completed',completedAt:now,completedBy:actor,completionSummary:summary},'WorkCompleted',actor,now,{completionSummary:summary??null,completionRecordId:this.eventId()}); }
  cancel(actor:WorkActor,reason:string,now:Date):void { this.requireMutable(['Draft','InProgress','WaitingForDecision']); if(!reason.trim()) throw new WorkInvariantViolation('Cancellation reason is required'); this.bump({...this.current,status:'Cancelled',cancelledAt:now,cancelledBy:actor,cancellationReason:reason.trim()},'WorkCancelled',actor,now,{reason:reason.trim()}); }
